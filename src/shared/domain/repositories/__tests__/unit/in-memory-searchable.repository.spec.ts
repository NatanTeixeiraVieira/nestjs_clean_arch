...
describe('InMemorySearchableRepository unit tests', () => {
  ...
  describe('applyFilter method', () => {
    ...
    it('should filter using a filter param', async () => {
      ...
      expect(itemsFiltered).toStrictEqual([items[0], items[1]]);
      expect(spyFilterMethod).toHaveBeenCalledTimes(2); // It was called 2 times

      itemsFiltered = await sut['applyFilter'](items, 'not-filter');
  ...
  describe('search method', () => {
    ...
    it('should apply pagination and sort', async () => {
      ...
      params = await sut.search(
        new SearchParams({ page: 3, perPage: 2, sort: 'name', sortDir: 'asc' }),
      );
      expect(params).toStrictEqual(
        new SearchResult({
          items: [items[3]],
    ...
    it('should search using pagination, sort and filter', async () => {
      ...
      let params = await sut.search(
        new SearchParams({ page: 1, perPage: 2, sort: 'name', filter: 'TEST' }),
      );
      expect(params).toStrictEqual(
        new SearchResult({
      ...
      params = await sut.search(
        new SearchParams({ page: 2, perPage: 2, sort: 'name', filter: 'TEST' }),
      );
      expect(params).toStrictEqual(
        new SearchResult({
          items: [items[2]],
